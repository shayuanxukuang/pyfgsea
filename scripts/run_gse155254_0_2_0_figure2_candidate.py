...
def _capture_release_git_state(expected_commit: str, expected_tag: str) -> dict[str, Any]:
    ...
    if RELEASE_TAG_PATTERN.fullmatch(expected_tag) is None:
        raise BindingError("--expected-git-tag must be v0.2.0-rcN or v0.2.0")
...
def _verify_artifact_receipt(
    ...
    if payload.get("status") != "passed" or payload.get("all_release_gates_passed") is not True:
        raise BindingError("artifact receipt did not pass every release gate")
    ...
    sdist_path = Path(str(sdist.get("path", ""))).resolve()
    ...
    if sdist.get("cargo_version") != EXPECTED_VERSION:
        raise BindingError("artifact sdist Cargo version is not 0.2.0")
    if sdist.get("metadata_version") != EXPECTED_VERSION:
        raise BindingError("artifact sdist metadata version is not 0.2.0")
    if wheel.get("metadata_version") != EXPECTED_VERSION:
        raise BindingError("artifact wheel metadata version is not 0.2.0")
    ...
    if installed.get("pyfgsea_version") != EXPECTED_VERSION:
        raise BindingError("installed artifact module version is not 0.2.0")
    if installed.get("distribution_version") != EXPECTED_VERSION:
        raise BindingError("installed artifact distribution version is not 0.2.0")
...
def _verify_installed_pyfgsea(artifact: Mapping[str, Any]) -> dict[str, Any]:
    ...
    installed = _require_mapping(artifact.get("installed"), "receipt installed artifact")
    if executable != Path(str(installed.get("python_executable", ""))).resolve():
        raise BindingError("current Python is not the verifier-created artifact environment")
    if prefix != Path(str(installed.get("sys_prefix", ""))).resolve():
        raise BindingError("current environment prefix differs from the artifact receipt")
    if pyfgsea.__version__ != EXPECTED_VERSION or distribution.version != EXPECTED_VERSION:
        raise BindingError("loaded PyFgsea module/distribution version is not 0.2.0")
...
def plot_target_curves(results: pd.DataFrame, png_path: Path, pdf_path: Path) -> None:
    ...
    fig.suptitle("PyFgsea 0.2.0 Figure 2 parameter-bound recalculation")
    fig.tight_layout()
