...
EXPECTED_CARGO_VERSION = "0.2.0-rc5"
...
BH_ATOL = 1e-14
RELEASE_TAG_PATTERN = re.compile(r"v0\.2\.0-rc5")
...
def _capture_release_git_state(expected_commit: str, expected_tag: str) -> dict[str, Any]:
    ...
    if RELEASE_TAG_PATTERN.fullmatch(expected_tag) is None:
        raise BindingError("--expected-git-tag must be v0.2.0-rc5")
...
def _load_reference_contract() -> dict[str, Any]:
    ...
    if any(current.get(key) != value for key, value in expected.items()):
        raise BindingError("current fgsea reference contract differs from the RC5 runner")
    return {
...
def plot_target_curves(results: pd.DataFrame, png_path: Path, pdf_path: Path) -> None:
    ...
    fig.suptitle("PyFgsea 0.2.0rc5 Figure 2 parameter-bound recalculation")
    fig.tight_layout()
