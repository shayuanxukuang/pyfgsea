...
RELEASE_TAG_PATTERN = re.compile(
    r"v(?P<cargo_version>(?P<base_version>(?:0|[1-9][0-9]*)\."
...
for label, installed_file in (
    ("pyfgsea", pyfgsea.__file__),
    ("pyfgsea._core", pyfgsea_core.__file__),
):
    resolved = pathlib.Path(installed_file).resolve()
    try:
        resolved.relative_to(venv_prefix)
    except ValueError as exc:
        raise RuntimeError(
            f"{label} resolved outside the fresh venv before test support was added: "
            f"{resolved}"
        ) from exc
...
def _verify_release_tag(repo: Path, release_tag: str, commit: str) -> dict[str, Any]:
    ...
    if tag_match is None:
        raise VerificationError(
            "--release-tag must be an annotated vMAJOR.MINOR.PATCH or "
            "vMAJOR.MINOR.PATCH-rcN tag"
        )
    ...
    return {
        "name": release_tag,
        "cargo_version": tag_match.group("cargo_version"),
        "base_version": tag_match.group("base_version"),
        "annotated": True,
...
def _distribution_version(cargo_version: str) -> str:
    rc_match = re.fullmatch(r"(\d+\.\d+\.\d+)-rc([1-9][0-9]*)", cargo_version)
...
def _committed_algorithm_revision(rust_source: bytes) -> str:
    ...
    if len(matches) != 1:
        raise VerificationError(
            "committed src/lib.rs must define exactly one string "
            "ALGORITHM_REVISION constant"
        )
    return matches[0]
...
def _verify_sdist(
    ...
    *,
    ...
    committed_cargo_version = _cargo_version(expected_sources["Cargo.toml"])
    if cargo_version != committed_cargo_version:
        raise VerificationError(
            f"sdist Cargo version is {cargo_version!r}, "
            f"expected committed version {committed_cargo_version!r}"
        )
...
def _require_portable_bundle_layout(output_dir: Path, receipt_path: Path) -> Path:
    """Require the uploaded ``dist/`` + ``evidence/`` bundle layout.

    Absolute paths remain useful as build-host provenance, but downstream
    consumers must be able to relocate a downloaded Actions artifact.  The
    receipt therefore lives at ``<bundle>/evidence/receipt.json`` and artifacts
    live in the sibling ``<bundle>/dist`` directory.
    """
    if receipt_path.parent.name != "evidence":
        raise VerificationError(
            "receipt must use the portable <bundle>/evidence/receipt.json layout"
        )
    bundle_root = receipt_path.parent.parent.resolve()
    expected_output_dir = (bundle_root / "dist").resolve()
    if output_dir.resolve() != expected_output_dir:
        raise VerificationError(
            "output directory must be the portable <bundle>/dist sibling of "
            "the receipt evidence directory"
        )
    return bundle_root
...
def _verify_installed_probe(
    ...
    *,
    ...
    venv_python_launcher = _venv_python(venv)
    expected_executable = venv_python_launcher.resolve()
    if executable != expected_executable:
        raise VerificationError(
            f"probe Python executable is {executable}, expected the interpreter "
            f"selected by the fresh venv launcher {venv_python_launcher} "
            f"(resolved to {expected_executable})"
        )
    ...
    return {
        ...
        "venv_python_launcher": str(venv_python_launcher.absolute()),
...
def _git_installed_test_manifest(repo: Path, commit: str) -> dict[str, bytes]:
    raw = _git_bytes(
        repo,
        "ls-tree",
        "-r",
    ...
    paths = [item.decode("utf-8", errors="strict") for item in raw.split(b"\0") if item]
    pipeline_test = INSTALLED_TEST_PATHS[1]
    if pipeline_test not in paths:
        raise VerificationError(
    ...
    for relative in sorted(paths):
        ...
        if path.is_absolute() or ".." in path.parts:
            raise VerificationError(f"unsafe installed test path: {relative!r}")
        committed = _git_bytes(repo, "show", f"{commit}:{relative}")
        worktree_path = repo.joinpath(*path.parts)
        if not worktree_path.is_file():
            raise VerificationError(
                f"installed test source is absent from the worktree: {relative!r}"
            )
        try:
            _git_bytes(
                repo,
            ...
            raise VerificationError(
                f"installed test source differs from commit: {relative!r}"
            ) from exc
        manifest[relative] = committed
    return manifest
...
def _junit_counts(path: Path) -> dict[str, int]:
    ...
    if root.tag == "testsuite":
        ...
        raise VerificationError(f"unexpected JUnit root element: {root.tag!r}")
    if not suites:
        raise VerificationError("installed-test JUnit XML contains no test suites")
    ...
    for suite in suites:
        for result_name, attribute in junit_attributes.items():
            raw_value = suite.get(attribute, "0")
            try:
                value = int(raw_value)
            except ValueError as exc:
                raise VerificationError(
                    f"invalid JUnit {attribute} count: {raw_value!r}"
                ) from exc
            if value < 0:
                raise VerificationError(
                    f"negative JUnit {attribute} count: {raw_value!r}"
                )
            totals[result_name] += value
    passed = totals["total"] - totals["failed"] - totals["errors"] - totals["skipped"]
    if passed < 0:
        raise VerificationError(f"inconsistent installed-test JUnit counts: {totals!r}")
    return {"passed": passed, **totals}


def _run_installed_tests(
    python: Path,
    *,
    repo: Path,
    commit: str,
    ...
    if _is_within(cwd, repo):
        raise VerificationError(
            f"installed tests must run outside the verified Git worktree: {cwd}"
        )
    _require_empty_or_absent(cwd, label="installed-test working directory")
    if junit_path.exists():
        raise VerificationError(
            f"installed-test JUnit evidence already exists: {junit_path}"
        )
    junit_path.parent.mkdir(parents=True, exist_ok=True)

    test_sources = _git_installed_test_manifest(repo, commit)
    version_probe = _run(
        [str(python), "-I", "-c", "import pytest; print(pytest.__version__)"],
        cwd=cwd,
        commands=commands,
        env=_isolated_python_environment(),
    )
    pytest_version = version_probe.stdout.decode("utf-8", errors="strict").strip()
    if pytest_version != INSTALLED_TEST_PYTEST_VERSION:
        raise VerificationError(
            f"installed pytest version is {pytest_version!r}, expected "
            f"{INSTALLED_TEST_PYTEST_VERSION!r}"
        )
    ...
    try:
        junit_bundle_path = (
            junit_path.resolve().relative_to(bundle_root.resolve()).as_posix()
        )
    except ValueError as exc:
        raise VerificationError(
            f"installed-test JUnit is outside the portable bundle: {junit_path}"
        ) from exc
    return {
        "status": "passed",
        "git_commit": commit,
        "test_paths": list(INSTALLED_TEST_PATHS),
        "test_source_manifest": _source_hash_manifest(test_sources),
        "test_source_manifest_sha256": _source_manifest_sha256(test_sources),
        "pytest_version": pytest_version,
        "trajectory_extra_installed": True,
        "isolated_python": True,
        "artifact_import_preloaded_before_test_support_path": True,
        "test_support_path_appended_after_site_packages": True,
        "import_mode": "importlib",
        "cwd": str(cwd.resolve()),
        "cwd_outside_worktree": True,
        "wheel_sha256": wheel_sha256,
        "junit": {
            "bundle_path": junit_bundle_path,
...
def _isolated_python_environment(
    *, disable_pytest_plugin_autoload: bool = False
    ...
    if disable_pytest_plugin_autoload:
...
def _build_and_verify(args: argparse.Namespace) -> dict[str, Any]:
    ...
    bundle_root = _require_portable_bundle_layout(output_dir, receipt_path)
    ...
    if release_tag["cargo_version"] != cargo_version:
        raise VerificationError(
            f"release tag version is {release_tag['cargo_version']!r}, "
            f"but committed Cargo version is {cargo_version!r}"
        )
    expected_version = _distribution_version(cargo_version)
    expected_algorithm_revision = _committed_algorithm_revision(
        expected_sources["src/lib.rs"]
    )
    ...
    sdist_evidence = _verify_sdist(
        sdist, expected_sources, expected_version=expected_version
    )
    ...
    try:
        ...
        wheel_evidence = _verify_wheel(
            wheel, expected_sources, expected_version=expected_version
        )
        wheel_evidence["build_input_sdist_sha256"] = sdist_evidence["sha256"]
        wheel_evidence["wheel_built_from_verified_sdist"] = True
        sdist_evidence["bundle_path"] = f"dist/{sdist.name}"
        wheel_evidence["bundle_path"] = f"dist/{wheel.name}"
        ...
        install_command.extend(
            [
                f"{wheel.resolve()}[trajectory]",
                f"pytest=={INSTALLED_TEST_PYTEST_VERSION}",
            ]
        )
        ...
        installed_tests_evidence = _run_installed_tests(
            venv_python,
            repo=repo,
            commit=commit,
            cwd=bundle_root / "installed-test-work",
            junit_path=receipt_path.parent / "installed-tests.junit.xml",
            bundle_root=bundle_root,
            wheel_sha256=str(wheel_evidence["sha256"]),
            commands=commands,
        )
    finally:
    ...
    return {
        ...
        "expected": {
            ...
            "algorithm_revision": expected_algorithm_revision,
        },
        "artifact_bundle": {
            "schema_version": 1,
            "layout": "receipt-parent-parent-v1",
            "bundle_root_relative_to_receipt": "..",
            "sdist": sdist_evidence["bundle_path"],
            "wheel": wheel_evidence["bundle_path"],
            "installed_tests_junit": installed_tests_evidence["junit"]["bundle_path"],
        },
        "git": {
            "repository": str(repo),
            "commit": commit,
        ...
        "installed_tests": installed_tests_evidence,
        "environment": {
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Annotated vMAJOR.MINOR.PATCH[-rcN] tag that must peel to --commit; "
            "the base version must match committed Cargo.toml"
        ),
    )
    parser.add_argument(
        "--output-dir",
