...
4. Figure 2 from the processed GSE155254 input through rolling-window GSEA. The manuscript's 500-cell baseline conflicts with the existing 400-cell/20-step/500-permutation generation script. On 2026-09-01 the project author accepted the explicit contract `window_size=500`, `step=50`, `nperm_nes=2000`, `score_type=std`, exact sizes (`mode=aligned`, `bin_width=0`), and no cross-window NES cache. The machine-readable contract is `repro/figure2_gse155254/author_parameter_contract.json`; accepting it does not accept the manuscript or publication. An earlier non-overwriting 0.2.0 candidate produced 62 windows by 43 pathways, all with `status=resolved`; all p-values were finite and positive, all recorded artifact hashes matched, and an independent within-window BH calculation agreed within the predeclared `1e-14` absolute tolerance. That candidate remains unbound because the source tree was dirty and its loaded native core was not tied to a verified release wheel. The pre-final-audit candidate is retained only as ignored local evidence; it is not part of this source revision. The versioned runner is `scripts/run_gse155254_0_2_0_figure2_candidate.py`; it requires explicit frozen H5AD/GMT paths, an external output directory, a clean unchanged annotated release tag, and a passed sdist-to-wheel artifact receipt whose installed wheel and native-core hashes match the active environment. A formal receipt must still be generated and attached to the release/archival record; no ignored `results/` path is cited as accepted evidence.
