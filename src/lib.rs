...
fn canonical_score_type(score_type: ScoreType) -> &'static str {
    match score_type {
        ScoreType::Std => "std",
        ScoreType::Pos => "pos",
        ScoreType::Neg => "neg",
        ScoreType::LegacyAbs => "two_sided_abs",
        ScoreType::LegacySigned => "one_sided_signed",
    }
}

fn canonical_tail_sign(score_type: ScoreType, sign: i32) -> i32 {
    match score_type {
        ScoreType::Std | ScoreType::LegacySigned if sign < 0 => -1,
        _ => 1,
    }
}
        ...
        .map(|(null_size, path_indices)| {
            ...
            for path_index in path_indices {
                ...
                let n_more = simple_null
                    ...
                    .count();
                if mode_count < 10 {
                    let reason = if mode == RunMode::Fast {
                        "fewer than 10 same-sign null samples; increase precheck_n"
                    } else {
                ...
                // Preserve the historical fast contract without ever reporting
                // a zero p-value: the raw empirical proportion is used only to
                // choose the shallow or deep path, while the reported shallow
                // estimate keeps the pseudocount above.
                let precheck_probability = n_more as f64 / mode_count as f64;
                let precheck_threshold = precheck_eps.unwrap_or(0.005);
                let legacy_forced = score_type == ScoreType::LegacyAbs;
                let fast_precheck_selected_simple =
                    mode == RunMode::Fast && precheck_probability > precheck_threshold;
                let aligned_error_selected_simple =
                    mode == RunMode::Aligned && mult_error >= simple_error(n_more, nperm_simple);
                let use_simple =
                    legacy_forced || fast_precheck_selected_simple || aligned_error_selected_simple;
                if use_simple {
                    let reason = if legacy_forced {
                        "legacy absolute-tail mode forced the simple estimator"
                    } else if fast_precheck_selected_simple {
            ...
            for (direction, pending) in pending_by_direction {
                ...
                for path in pending {
                    ...
                    let selection_reason = if mode == RunMode::Fast {
                    ...
                    if let Some(unconditional_log) = query {
                        let reason =
                            format!("{selection_reason}; multilevel compound boundary resolved");
                        let denominator = if require_mode {
                        ...
                        let reason = format!("{selection_reason}; {}", outcome.reason);
                        resolved.push((
                            path.index,
                            resolved_from_log(
                                observation,
...
fn build_tail_curve_inner(
    ...
    let canonical_sign = canonical_tail_sign(parsed, sign);
    let mut rng = ChaCha20Rng::seed_from_u64(seed);
    ...
    for _ in 0..sample_size {
        ...
        let metric = tail_metric(parsed, standard, positive, negative, canonical_sign);
    ...
    Ok(TailCurve {
        ...
        termination_reason: "legacy empirical tail curve; aligned mode uses the exact ruler"
            .to_string(),
        score_type: canonical_score_type(parsed).to_string(),
        sign: canonical_sign,
        approximate: true,
        algorithm_revision: "legacy-empirical-tail-v1".to_string(),
    })
}
...
fn query_tail_curve(
    ...
    let requested_score_type = score_type.unwrap_or(&curve.score_type);
    let parsed = parse_score_type(requested_score_type).map_err(PyValueError::new_err)?;
    let canonical_score_type = canonical_score_type(parsed);
    if canonical_score_type != curve.score_type {
        return Err(PyValueError::new_err(format!(
            "score_type mismatch: curve was built with {:?}, query requested {:?}",
            curve.score_type, canonical_score_type
        )));
    }
    let canonical_sign = canonical_tail_sign(parsed, sign.unwrap_or(curve.sign));
    if canonical_sign != curve.sign {
        return Err(PyValueError::new_err(format!(
            "sign mismatch: curve was built with {}, query requested {}",
            curve.sign, canonical_sign
        )));
    }
    let metric = observed_tail_metric(parsed, obs_es, canonical_sign);
    ...
    Ok((
        ...
        simple_log2_error(count, population.len()),
    ))
}
...
impl GseaPrerankedRunner {
    ...
    fn new(pathways: Vec<Vec<usize>>, min_size: usize, max_size: usize) -> PyResult<Self> {
        ...
        let mut prepared = Vec::with_capacity(pathways.len());
        for (index, mut pathway) in pathways.into_iter().enumerate() {
            pathway.sort_unstable();
            pathway.dedup();
            let size = pathway.len();
            if size < min_size || size > max_size {
                return Err(PyValueError::new_err(format!(
                    "pathway {index} has deduplicated size {size}, outside [{min_size}, {max_size}]"
                )));
            }
            prepared.push(pathway);
        }
        Ok(Self { pathways: prepared })
    }
...
#[pyfunction]
#[pyo3(signature = (scores, pathways, sample_size, seed, gsea_param, eps, score_type=None, bin_width=None, mode="aligned", nperm_simple=None, max_levels=None, precheck_n=None, precheck_eps=None))]
#[allow(clippy::too_many_arguments)]
...
#[pyfunction]
#[pyo3(signature = (scores, pathways, sample_size, seed, gsea_param, eps, score_type=None, mode="aligned", nperm_simple=None, max_levels=None, precheck_n=None, precheck_eps=None))]
#[allow(clippy::too_many_arguments)]
fn fgsea_multilevel(
    scores: PyReadonlyArray1<f64>,
    pathways: Vec<Vec<usize>>,
