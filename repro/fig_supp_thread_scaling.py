...
def plot_thread_scaling_supplement():
    ...
    ax1.plot(df["Threads"], df["Speedup"], marker='o', linestyle='-', color=color, linewidth=2, label="Speedup")
    ax1.tick_params(axis='y', labelcolor=color)
    ax1.set_xticks(df["Threads"])
    
    # Ideal Linear Speedup (Reference)
    ax1.plot([1, 16], [1, 16], linestyle='--', color='gray', alpha=0.5, label="Ideal Linear")
    
    # Memory Curve (Secondary Axis)
    ax2 = ax1.twinx()
    color = 'tab:red'
    ax2.set_ylabel('Peak RSS (MB)', color=color)
    ax2.plot(df["Threads"], df["Memory"], marker='s', linestyle='--', color=color, linewidth=2, label="Memory")
    ...
    if 8 in df["Threads"].values:
        ...
        if speedup_8 < 6: 
            ax1.annotate("Diminishing Returns", xy=(8, speedup_8), 
