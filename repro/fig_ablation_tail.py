...
if not have_rscript():
    print(
        "[WARN] Rscript not found. Skipping R baseline. (Install R or set up conda r-base)"
    )
    run_r = False
else:
    run_r = True
...
def main():
    ...
    if run_r:
        ...
        if not r_res_file.exists():
            print(
                "Error: R results not found despite running R script. Skipping comparison."
            )
            run_r_comparison = False
        else:
            run_r_comparison = True
    else:
        print("  Skipping R baseline (Rscript not found).")
        run_r_comparison = False
    ...
    if run_r_comparison:
        ...
        # Save PyFgsea only results
        print("\n[Analysis] Saving PyFgsea results (R comparison skipped).")
        res_py.to_csv(out_dir / "pyfgsea_results.csv")
        print(f"  Saved PyFgsea results to {out_dir / 'pyfgsea_results.csv'}")
