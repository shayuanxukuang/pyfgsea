...
def run_tool(tool_name, df_rnk, gmt_obj):
    if tool_name == "PyFgsea":
        ...
        gp.prerank(rnk=df_rnk, gene_sets=gmt_obj, threads=4, min_size=15, max_size=2000, seed=42, verbose=False)
    elif tool_name == "BlitzGSEA":
        if blitzgsea is None: return
        sig_copy = df_rnk[["Gene", "Score"]].copy()
        # BlitzGSEA uses multiprocessing
        blitzgsea.gsea(sig_copy, gmt_obj, min_size=15, max_size=2000, seed=42, verbose=False, processes=4)

def main():
    # Enforce thread consistency
    for env_var in ["RAYON_NUM_THREADS", "OMP_NUM_THREADS", "NUMEXPR_NUM_THREADS", "MKL_NUM_THREADS"]:
    ...
    for scen in scenarios:
        ...
        for tool in tools:
            ...
            if status == "Success":
                print(f"{scen['desc']:<10} {tool:<10} {duration:<10.2f} {ram_mb:<10.2f}")
            else:
