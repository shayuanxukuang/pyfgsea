...
def run_ablation_study():
    ...
    for name, kwargs, sort_mode in configs:
        ...
        if sort_mode == "stable":
            ...
            df_run = df_run.sample(frac=1, random_state=42).sort_values("Score", kind='mergesort', ascending=False)
        else:
            df_run = df_run.sort_values("Score", ascending=False)
            
        # Run PyFgsea
        res = run_gsea(df_run, gmt, gene_col="Gene", score_col="Score", seed=42, **kwargs)
        ...
        if name == "Stable + Eps=0":
            ...
            if outliers:
                print(f"    Found {len(outliers)} outliers in 'Stable + Eps=0': {outliers[:3]}...")
                target = outliers[0]
                print(f"    Tracing outlier: {target}")
                print(f"      R NES: {r_sub.loc[target, 'NES']:.4f}, Py NES: {py_sub.loc[target, 'NES']:.4f}")
                print(f"      R Pval: {r_sub.loc[target, 'pval']:.4e}, Py Pval: {py_sub.loc[target, 'pval']:.4e}")
