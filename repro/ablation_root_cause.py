...
def run_r_baseline(
    df, gmt, out_path, reference: RReferenceEnvironment, eps=0
):
...
def run_ablation_study():
    ...
    write_reference_environment(
        ...
        reference,
        {"script": "repro/ablation_root_cause.py"},
    )
    ...
    run_r_baseline(df_raw, gmt, r_out, reference, eps=0)

    if not r_out.exists():
        raise RuntimeError("R reference completed without producing r_baseline.csv")
