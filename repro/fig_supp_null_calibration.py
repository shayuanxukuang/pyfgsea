...
def run_multi_null_calibration_supplement():
    ...
    for r in range(n_reps):
        ...
        res = run_gsea(df_rank, gmt, use_batched=False, seed=seed, min_size=15, max_size=500)
        ...
        if not res.empty:
            ...
            ax1.plot(-np.log10(expected), -np.log10(p_sorted), color='gray', alpha=0.3, linewidth=1)
    ...
    sns.ecdfplot(all_pvals, ax=ax2, color='blue', label=f'Empirical (N={len(all_pvals)})')
