...
def run_window_sensitivity_supplement():
    ...
    if root_gene not in adata_sub.var_names:
        root_gene = "KIT"
    if root_gene not in adata_sub.var_names:
        raise ValueError("Paul15 erythroid subset contains neither GATA2 nor KIT")
    ...
    for w in windows:
        ...
        if df.empty:
            raise RuntimeError(f"Trajectory GSEA produced no results for window_size={w}")
        sub = df[df["Pathway"].str.contains(target_pattern, case=False)].copy()
        if sub.empty:
            raise ValueError(
                f"No {target_pattern} result exists for window_size={w}"
            )
        # Normalize x-axis
    ...
    if not dfs:
        raise RuntimeError("No window-sensitivity results were collected")
