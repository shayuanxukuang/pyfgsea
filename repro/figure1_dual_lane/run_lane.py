...
def _verify_artifact_receipt(
    ...
    if lane == "legacy":
        ...
        if receipt.get("all_release_gates_passed") is not True:
            raise ValueError("current artifact receipt did not pass all release gates")
        if receipt.get("expected") != {
...
def _validate_input_manifest(
    ...
    if historical.get("source_commit") != PUBLICATION_SOURCE_COMMIT:
        raise ValueError("input manifest cites the wrong historical source commit")
    ...
    for name in SCENARIOS:
        ...
        for label, record in (("ranks", ranks_record), ("pathways", pathways_record)):
            if record.get("sha256") != EXPECTED_INPUT_SHA256[name][label]:
                raise ValueError(
                    f"{name}/{label} hash differs from the tagged contract"
                )
        resolved[name] = {
...
def run_lane(args: argparse.Namespace) -> Path:
    ...
    for scenario_name in SCENARIOS:
        ...
        r_stdout_path.write_text(completed.stdout, encoding="utf-8", newline="\n")
        r_stderr_path.write_text(completed.stderr, encoding="utf-8", newline="\n")
