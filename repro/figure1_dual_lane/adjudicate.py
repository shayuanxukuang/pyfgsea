#!/usr/bin/env python3
"""Combine two verified lane receipts and derive every Figure 1 metric from raw rows."""
...
def _validate_receipt(
    ...
    if receipt.get("lane_contract") != LANE_CONTRACTS[lane]:
        raise ValueError(f"{lane} receipt contract differs from the adjudicator")
    ...
    if not isinstance(artifact_binding, Mapping) or not isinstance(
        ...
        raise ValueError(f"{lane} identity lacks artifact/reference bindings")
    ...
    if sha256_file(artifact_copy) != artifact_binding.get(
        ...
        raise ValueError(
            f"{lane} upstream receipt copies differ from identity bindings"
        )
    for scenario in SCENARIOS:
...
def _verify_common_input_bundle(
    ...
    for lane, receipt in (("legacy", legacy_receipt), ("current", current_receipt)):
        ...
        if recorded != manifest_hash:
            raise ValueError(
                f"{lane} receipt does not bind the supplied input manifest"
            )
        ...
        for label in ("ranks", "pathways"):
            ...
            for lane, receipt in (
                ...
                if recorded != file_hash:
                    raise ValueError(
                        f"{lane} receipt does not bind input bundle {scenario}/{label}"
                    )
...
def _draw_figure(raw: pd.DataFrame, metrics: pd.DataFrame, output: Path) -> None:
    ...
    for axis, lane, scenario in zip(
        ...
        lane_label = (
            "0.1.4 / fgsea 1.32.2" if lane == "legacy" else "0.2.0 / fgsea 1.38.0"
        )
        scenario_label = (
            "publication input"
...
def adjudicate(args: argparse.Namespace) -> Path:
    ...
    if _is_within(output_dir, REPO_ROOT):
        raise ValueError(
            "formal adjudication outputs must be outside the evidence checkout"
        )
